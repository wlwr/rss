var request = require('../request');
var redis = require('../redis');
var xmlParser = require('xml2json');
var util = require('util');
var config = require('../../config/weixin');

function Weixin(id) {
  this.id = id;
}

Weixin.prototype.getData = function*() {
  var rss = {};
  //list-cache
  var listCacheId = util.format(config.listCachePolicy.cacheId, this.id);
  var listCache =
    yield redis.get(listCacheId);
  if (config.cacheEnable && listCache && listCache.items)
    return JSON.parse(listCache);
  //list-collect
  var apiUrl = util.format(config.api.getList, this.id, new Date().getTime());
  rss.items = yield this.getListData(apiUrl);
  for (var i = 0, l = rss.items.length; i < l; i++) {
    var item = rss.items[i];
    //content-cache
    var contentCacheId = util.format(config.contentCachePolicy.cacheId, item.docid);
    var content =
      yield redis.get(contentCacheId);
    if (!config.cacheEnable || !content) {
      //content-collect
      content = yield this.getContentData(item);
      if (content)
        yield redis.setex(contentCacheId, config.contentCachePolicy.lifeTime, content);
    }
    item.fullContent = content;
  };
  if (rss.items[0]) {
    var recentItem = rss.items[0];
    rss.id = this.id;
    rss.url = apiUrl;
    rss.title = recentItem.sourcename;
    rss.updated = recentItem.date;
    rss.author = recentItem.sourcename;
  }
  yield redis.setex(listCacheId, config.listCachePolicy.lifeTime, JSON.stringify(rss));
  return rss;
};


Weixin.prototype.getListData = function*(apiUrl) {
  var items = [];
  var cookie = yield this.getRandomCookie();
  if (!cookie) {
    var cookies = yield this.getCookies();
    yield this.saveCookies(cookies);
    cookie = cookies[0];
  }
  var response =
    yield request({
      url: apiUrl,
      headers : this.getRequestHeaders(cookie)
    });
  if (response.error)
    return {
      error: response.error
    };
  if (response.statusCode != 200)
    return {
      error: 'response statusCode: ' + response.statusCode
    }
  var body = response.body;
  var leftLimit = body.indexOf('({');
  var rightLimit = body.indexOf('})');
  var dataJson = JSON.parse(body.slice(leftLimit + 1, rightLimit + 1));
  dataJson.items.forEach(function(item) {
    var itemJson = JSON.parse(xmlParser.toJson(item));
    var _item = itemJson.DOCUMENT.item.display;
    items.push(_item);
  });
  return items;
}


Weixin.prototype.getContentData = function*(item) {
    var result =
      yield request({
        url: item.url
      });
    if (!result.error && result.statusCode == 200) {
      var match = result.body.match(/id=\"js_content\">(.*)<\/div>/);
      if (match && match[1]) {
        content = match[1].replace(/data-src/g, 'src');
        return content;
      }
    }
}

Weixin.prototype.getCookies = function*() {
  var cookies = [];
  for (var i = 0, l = config.cookieNum; i < l; i++) {
    var cookie = yield this.getCookie();
    if (cookie)
      cookies.push(cookie);
  };
  return cookies;
}

Weixin.prototype.saveCookies = function*(cookies) {
    yield redis.sadd(config.cookieCachePolicy.cacheId, cookies);  
    yield redis.expire(config.cookieCachePolicy.cacheId, config.cookieCachePolicy.lifeTime);
}

Weixin.prototype.getCookie = function*() {
  var headerCookie = 'ABTEST=0|1430540424|v1; SNUID=A596C73147425256E3BB03EE47618F92; IPLOC=CN4401; SUID=E2D081776A20900A0000000055445088; SUID=E2D081776914920A000000005544508B; SUV=00E979B57781D0E25544508BAF939158; SUIR=1430540428; PHPSESSID=9it6n9jmn65099uu6lgg2d0go5';
  var result = yield request({
    url: util.format(config.api.getCookie, Math.random().toString(36).substr(2)),
    headers: this.getRequestHeaders(headerCookie)
  });
  if (!result.error && result.statusCode == 200) {
    var cookieStr = result.headers['set-cookie'].join(';');
    console.log(cookieStr);
    var SNUID = cookieStr.match(/(SNUID=\S+?);/)[1];
    var SUID = cookieStr.match(/(SUID=\S+?);/)[1];
    var SUV = 'SUV=' + ((new Date()).getTime())*1000+Math.round(Math.random()*1000);
    var cookie = [SNUID, SUID, SUV].join(';');
    return cookie;
  }  
}

Weixin.prototype.getRandomCookie = function*() {
  return yield redis.srandmember(config.cookieCachePolicy.cacheId);
}

Weixin.prototype.getRequestHeaders = function(cookie) {
  return {
    'Accept': '*/*',
    'Accept-Language': 'zh-CN,zh;q=0.8,en;q=0.6',
    'Cache-Control': 'max-age=0',
    'Connection': 'keep-alive',
    'Cookie': cookie,
    'Host': 'weixin.sogou.com',
    'Referer': 'http://weixin.sogou.com/gzh?openid=oIWsFtzoNLI6-6I8GNfFgODdmXAQ',
    'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.135 Safari/537.36'
  }
}

module.exports = Weixin;